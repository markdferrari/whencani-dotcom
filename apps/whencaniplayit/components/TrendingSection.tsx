...
function TrendingSectionStandard({ games }: { games: TrendingGame[] }) {
  return (
    <MediaCarousel
      label="Trending"
...
function TrendingSectionLegacy({ games }: { games: TrendingGame[] }) {
      ...
      <div className="relative mt-5 group">
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-4">
            {games.map((game) => (
              <div key={`${game.id}-carousel`} className="min-w-0 flex-[0_0_100%]">
...
export function TrendingSection() {
  ...
  if (config.features.standardCarousels) {
    return <TrendingSectionStandard games={games} />;
  }

  return <TrendingSectionLegacy games={games} />;
}
