...
export function useWatchlistActions() {
  ...
  return useCallback(async (movieId: number, action: WatchlistAction) => {
...
export function useWatchlistMovies() {
  ...
  const fetchMovies = useCallback(async () => {
    ...
    try {
      ...
      const data = (await response.json()) as { movies?: TMDBMovie[] };
  ...
  return { movies, isLoading, refresh: fetchMovies };
}
