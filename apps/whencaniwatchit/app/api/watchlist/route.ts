...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ids, movies });
  } catch (error) {
    console.error('Failed to fetch watchlist', error);
    return NextResponse.json({ ids, movies: [] }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  if (!Number.isFinite(movieId)) {
    return NextResponse.json({ message: 'Invalid movie identifier' }, { status: 400 });
  }

  const currentIds = readWatchlistFromRequest(request);
  const updatedIds = action === 'add'
    ? addToWatchlist(currentIds, movieId)
    : removeFromWatchlist(currentIds, movieId);
